# -*- coding: utf-8 -*-
"""生成《AI预打分方案_v5.docx》到 D:\\wqgao\\code\\AI_scoring_2。

v5 相对 v4 的变更（用户要求）：
1. 锚点档位可配置（3/6/9）：第三章改多锚点列表结构与档位表（--count）
2. 第八章「比较与分数映射」：两段插值 → N 段分段线性插值
3. 分布校准按标签定位（修复 fair 锚点偏差为 0 时索引错位）
"""
...
OUT = Path(__file__).parent / "AI预打分方案_v5.docx"
...
# ============ 封面标题 ============
title = DOC.add_heading("AI 预打分方案（版本 5）", level=0)
for run in title.runs:
    run.font.name = "微软雅黑"
    run._element.rPr.rFonts.set(qn("w:eastAsia"), "微软雅黑")
sub = DOC.add_paragraph()
run = sub.add_run(
    "基于分割图几何特征（逐笔颜色比对）的批量预打分方案。v5 新增：多锚点档位可配置（3/6/9）、N 段分段插值、校准标签定位修复。"
)
run.font.size = Pt(10)
...
para("每个字一个锚点目录，目录名即字名。锚点档位可配置（3/6/9 个），用户按模板放置分割图，刻度配置由程序生成默认值、按需修改。")
heading("3.1 目录与命名模板", 2)
code("AI_scoring_2\\data\\anchors\\{字}\\")
code("├── perfect.png      # 标准满分分割图（首档，必填）")
code("├── level_1.png      # 中间档（仅 6/9 档时存在，由好到差排列）")
code("├── ...              # level_N.png ...")
code("├── worst.png        # 最差分割图（末档，明显差即可）")
code("└── anchor.json      # 刻度配置（由 init_anchor.py --count 生成，可按需修改）")
para("3 档时中间档文件名为 fair.png（兼容旧目录）；文件名以 anchor.json 的 file 字段为准，可自定义。")
...
table(
    ...
    [
        ...
        ["命名", "默认 perfect.png / fair(level_*).png / worst.png；实际以 anchor.json 的 file 字段为准"],
    ],
)
heading("3.3 anchor.json 结构（多锚点列表，兼容旧三键）", 2)
code('{')
code('  "char": "刀",                        // 字名，须与目录名一致')
code('  "version": "2",')
code('  "anchors": [                         // 有序列表，第 0 个为满分锚点')
code('    {"file": "perfect.png", "score_ratio": 1.0,   "label": "perfect"},')
code('    {"file": "level_1.png", "score_ratio": 0.75,  "label": ""},')
code('    {"file": "level_2.png", "score_ratio": 0.55,  "label": ""},')
code('    {"file": "level_3.png", "score_ratio": 0.425, "label": "fair"},')
code('    {"file": "level_4.png", "score_ratio": 0.25,  "label": ""},')
code('    {"file": "worst.png",   "score_ratio": 0.125, "label": "worst"}')
code('  ],')
code('  "dimension_overrides": {}             // 可选：某维度指定不同锚点样本或覆盖比例')
code('}')
para("兼容旧版三键 dict 结构（perfect/fair/worst），程序自动转为列表；label 用于识别 fair/worst 语义（分布校准只放宽这两处）。")
heading("3.4 档位配置与生成流程", 2)
table(
    ["步骤", "动作"],
    [
        ["1. 建目录", "运行 init_anchor.py --char 刀 --count 3/6/9，自动创建 data\\anchors\\刀\\ 并按档位表生成 anchor.json"],
        ["2. 放图", "按 anchor.json 的 file 字段放入 N 张分割图（由好到差）"],
        ["3. 校验", "运行校验：文件存在、PNG 可读、颜色数 ≥ 2（非空白）；失败则拒绝该字进入评分"],
        ["4. 可选调参", "按需修改 anchor.json 的 score_ratio、label 或 dimension_overrides"],
    ],
)
para("档位比例表（config.json → anchor_defaults.score_levels）：3 档 [1.0, 0.425, 0.125]；6 档 [1.0, 0.75, 0.55, 0.425, 0.25, 0.125]；9 档 [1.0, 0.85, 0.7, 0.55, 0.425, 0.3, 0.2, 0.125, 0.05]。中间档图只需「由好到差」梯度，程序自动按偏差定位刻度。")
...
# ============ 八、比较与分数映射 ============
heading("八、比较与分数映射", 1)
para("每个维度独立执行多锚点分段线性插值（锚点档位数 3/6/9 可配置）：")
code("样本偏差 d（相对满分锚点的多维加权距离）在 N 个锚点间定位：")
code("  满分锚点 d=0 → 满分 Max；最差锚点 d_worst → 该维满分 × worst.score_ratio（默认 12.5%）")
...
bullet("锚点档位、score_ratio 全部来自各字 anchor.json（--count 生成），全局默认在 config.json 的 anchor_defaults.score_levels，均可调。")
bullet("分布校准（可选）：当样本偏差普遍超过锚点刻度时，把 fair（50 分位）与 worst（90 分位）两处刻度放宽到样本分位数，防止分数分布被压死；按标签定位，多档下只干预这两处。")
