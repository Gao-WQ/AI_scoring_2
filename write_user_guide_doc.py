...
run = sub.add_run(
    "基于分割图几何特征（逐笔颜色比对）+ 多档位锚点刻度插值（3/5/9 可调）的手写汉字六维预打分系统。"
    "为评分老师提供可修改的初稿，降低人工评分工作量。版本 v5（锚点档位 3/5/9 + --anchor-count 切换），更新记录见 CHANGELOG.md。"
)
run.font.size = Pt(10)
...
for tip in [
    ...
    "提取六维特征（笔画/结构/位置/占格/衔接/留白）→ 与锚点图池（1.png~9.png，1=最佳 / 9=最差）做偏差比较",
    "按档位取锚点子集做分段线性插值 → 六维分（0~15/20/15/10/10/10）→ 写入 Excel，总分由公式自动求和",
]:
    p = para("• " + tip)
    p.paragraph_format.left_indent = Pt(12)

para("锚点档位：3/5/9 三档可选，共用同一套 1.png~9.png 图池；跑分时用 --anchor-count 指定用哪一档，档位与 ratio 可在 config.json → anchor_defaults 中调整。")
table(
...
code("├── data\\")
code("│   ├── anchors\\{字}\\        # 锚点（1~9.png 图池 + anchor3/5/9.json）")
...
table(
    ...
    [
        ...
        ["src/init_anchor.py", "生成锚点目录模板（anchor{count}.json，--count 3/5/9）", "init-anchor"],
        ...
        ["src/common/anchor_utils.py", "锚点模板创建、anchor{count}.json 分档解析、锚点目录校验", "—"],
        ...
        ["src/scoring/score_mapper.py", "N 档锚点分段插值 → 六维分；批量映射 + 分布校准", "—"],
...
code("python main.py init-anchor --char 刀           # 3 档（默认，取 config.anchor_defaults.anchor_count）")
code("python main.py init-anchor --char 刀 --count 5 # 5 档")
code("python main.py init-anchor --char 刀 --count 9 # 9 档")
table(
    ["参数", "默认", "说明"],
    [
        ["--char", "刀", "字名（目录名）"],
...
para("产出 data\\anchors\\{字}\\anchor{count}.json（N 条锚点，file = 对应图序号）。图池规范：1.png=最佳(perfect)、9.png=最差(worst)、5.png=中等(fair)；3/5/9 档三份 json 可并存，跑分时用 --anchor-count 选档。score_ratio 与 label 已自动填好，可改。")
...
code("python main.py run-all --char 上 --save-features --anchor-count 9   # 单字 + 9 档")
code("python main.py run-all                              # 全量（扫描源目录），默认档位取 config")
code("python main.py run-all --chars-file data\\chars.txt --anchor-count 5 # 指定清单 + 5 档")
table(
    ["参数", "默认", "说明"],
    [
        ["--char", "None", "只处理单字"],
...
para("产出：data\\scores\\{字}_scores.json、data\\output\\{字}{input_suffix}{output_suffix}.xlsx、summary.csv。缺锚点（该档 json 或对应图缺失）的字跳过不阻塞。")
para("换数据源：修改 config.json 的 source_dir（目录）与 paths.input_suffix（后缀，如 _all_data_new 或 -打分表-1），源文件名 = {字}{input_suffix}.xlsx，代码不用改。")
para("运行时输出：每字打印 [处理] 字=X | 锚点=N档 | 待打分样本量=N | 打分表保存: ...；样本量 >50 时特征提取阶段显示终端进度条。")
...
code("  → map_scores_batch（按档位锚点子集偏差插值 + 分布校准 → 六维分）")
...
code("python main.py init-anchor --char 新字 --count 9    # 生成模板；可分别 --count 3/5/9 生成三档并存")
para("→ 放置图池 1.png~9.png（1 最好 → 9 最差，由好到差）；同一套图可支撑 3/5/9 三档 json")
code("python main.py stroke-check --char 新字 --expected-strokes N")
code("python main.py run-all --char 新字 --save-features --anchor-count 9")
para("→ 检查 data\\output\\ 成品 + summary.csv；对比不同档位效果时改 --anchor-count 重跑即可")
