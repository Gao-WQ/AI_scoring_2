# -*- coding: utf-8 -*-
"""生成《AI预打分方案_v3.docx》到 D:\\wqgao\\code\\AI_scoring_2。

v3 相对 v2 的变更（用户要求）：
1. 新增「代码约束规范」：公共函数收敛到 common/ 独立文件，业务脚本只调用不复制，杜绝冗余代码
2. 打磨第四章：src 从扁平结构升级为分层结构（common/features/scoring/pipeline），明确职责与单向依赖
"""
...
OUT = Path(__file__).parent / "AI预打分方案_v3.docx"
...
# ============ 封面标题 ============
title = DOC.add_heading("AI 预打分方案（版本 3）", level=0)
for run in title.runs:
    run.font.name = "微软雅黑"
    run._element.rPr.rFonts.set(qn("w:eastAsia"), "微软雅黑")
sub = DOC.add_paragraph()
run = sub.add_run(
    "基于分割图几何特征（逐笔颜色比对）的批量预打分方案。v3 新增：代码约束规范（公共函数收敛 common/）、src 分层目录结构。"
)
run.font.size = Pt(10)
